...
__global__ void phase3(int *global_Dist, int round, int matrixSize, int start){
    __shared__ int share_Dist[BLOCKFACTOR][BLOCKFACTOR * 2];
    int local_round = round;
    int j = threadIdx.x;
    int i = threadIdx.y;
    int block_j = blockIdx.x;
    int block_i = blockIdx.y + start;
...
void block_FW() {
	...
	#pragma omp parallel num_threads(GPU_NUM)
	{
        ...
        cudaMalloc(&Dev_Dist[gpu_id], n * n * sizeof(int));
        ...
        dim3 grid1(1);
        dim3 grid2(2, round);
        dim3 grid3(round, end_x-start_x);

        
        cudaMemcpy(Dev_Dist[gpu_id] + start_x * BLOCKFACTOR * n, Dist + start_x * BLOCKFACTOR * n, BLOCKFACTOR * n * (end_x - start_x) * sizeof(int), cudaMemcpyHostToDevice);
        ...
        for(int r=0;r<round/2;r++){
            int target_id = 0;
            if(gpu_id == target_id){
                cudaMemcpy(Dist + r * BLOCKFACTOR * n, Dev_Dist[gpu_id] + r * BLOCKFACTOR * n, BLOCKFACTOR * n * sizeof(int), cudaMemcpyDeviceToHost);
            }
            #pragma omp barrier
            if(gpu_id != target_id){
                cudaMemcpy(Dev_Dist[gpu_id] + r * BLOCKFACTOR * n, Dist + r * BLOCKFACTOR * n, BLOCKFACTOR * n * sizeof(int), cudaMemcpyHostToDevice);
            }
            phase1 <<<grid1, block>>>(Dev_Dist[gpu_id], r, n);
            phase2 <<<grid2, block>>>(Dev_Dist[gpu_id], r, n);
            phase3 <<<grid3, block>>>(Dev_Dist[gpu_id], r, n, start_x);
        }
		...
		cudaMemcpy(Dist + start_x * BLOCKFACTOR * n, Dev_Dist[gpu_id] + start_x * BLOCKFACTOR * n, BLOCKFACTOR * n * (end_x - start_x) * sizeof(int), cudaMemcpyDeviceToHost);
    }
    cudaFree(Dev_Dist);
}
