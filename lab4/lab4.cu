...
__global__ void sobel(unsigned char *s, unsigned char *t, unsigned height, unsigned width, unsigned channels) {
    ...
    for (int y=blockIdx.x; y<height; y+=gridDim.x) {
            ...
            for (int v = -yBound; v <= yBound; ++v) {
                for (int u = -xBound; u <= xBound; ++u) {
                    if (bound_check(x + u, 0, width) && bound_check(y + v, 0, height)) {
                        ...
                        v02 += R * mask[0][u + xBound][v + yBound];
                        v01 += G * mask[0][u + xBound][v + yBound];
                        v00 += B * mask[0][u + xBound][v + yBound];
                        v12 += R * mask[1][u + xBound][v + yBound];
                        v11 += G * mask[1][u + xBound][v + yBound];
                        v10 += B * mask[1][u + xBound][v + yBound];
                    }
                }
            }
        //}
        ...
        float totalB = 0.;
        //for (int i = 0; i < Z; ++i) {
            totalR = (v02*v02+v12*v12);
            totalG = (v01*v01+v11*v11);
            totalB = (v00*v00+v10*v10);
        //}
